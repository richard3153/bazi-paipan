...
class InterpretationResult:
    """解读结果数据结构"""
    day_master_analysis: Dict  # 日主分析
    wuxing_analysis: Dict  # 五行分析
    shishen_analysis: Dict  # 十神分析
    geshi_analysis: Dict  # 格局分析
    tiaohou_analysis: Dict  # 调候分析
    yongshen_analysis: Dict  # 用神分析
    fortune_analysis: Dict  # 运势分析
    suggestions: Dict  # 改善建议
...
class InterpretationEngine:
    ...
    def _determine_shishen(self, dm_wx: str, other_wx: str, dm_idx: int, other_idx: int) -> str:
        ...
        if sheng_relation[dm_wx] == other_wx:
            # 阴阳相同为食神，不同为伤官
    ...
    def _analyze_wuxing(self, chart: BaZiChart) -> Dict:
        ...
        wuxing_count[month_wuxing] += 2  # 月令权重最高
    ...
    def _analyze_tiaohou(self, chart: BaZiChart) -> Dict:
        ...
        # 检查命局是否有调候用神
        has_tiaohou = self._check_yongshen_in_chart(chart, tiaohou_yongshen)
        ...
        return {
            ...
            "principle": principle,
            "taboo": taboo,
            "has_tiaohou_in_chart": has_tiaohou,
            "priority": "最高",
    ...
    def _check_yongshen_in_chart(self, chart: BaZiChart, yongshen: str) -> bool:
        """检查命局中是否有用神"""
        # 简化处理：检查天干地支中是否有用神五行
        yongshen_wuxing = yongshen.replace("（调候）", "").replace("（暖局）", "").replace("（润局）", "").strip()
    ...
    def _analyze_yongshen(self, chart: BaZiChart, tiaohou_analysis: Dict,
        ...
        # 4. 通关用神（辅助）
        # 检查是否有两神对峙
        tongguan = self._check_tongguan(wuxing_analysis)
        if tongguan:
            yongshen_list.append({
                "type": "通关用神",
                "yongshen": tongguan,
            ...
            analysis_details["tongguan"] = tongguan
    ...
    def _get_supporting_wuxing(self, dm_wx: str) -> List[str]:
        ...
        same_wuxing = [dm_wx]
        return sheng_relation.get(dm_wx, "") + same_wuxing

    def _check_tongguan(self, wuxing_analysis: Dict) -> Optional[str]:
        """检查是否需要通关用神"""
        wuxing_count = wuxing_analysis.get("wuxing_count", {})

        # 简化判断：如果有两个五行特别旺且相克，需要通关
       旺五行 = [wx for wx, count in wuxing_count.items() if count >= 3]

        if len(旺五行) >= 2:
            # 检查是否相克
            for i, wx1 in enumerate(旺五行):
                for wx2 in 旺五行[i + 1:]:
                    if self._is_ke(wx1, wx2):
                        # 需要通关
                        tongguan_wx = self._get_tongguan_wuxing(wx1, wx2)
                        return tongguan_wx
    ...
    def _is_ke(self, wx1: str, wx2: str) -> bool:
        """判断五行是否相克"""
        ke_relation = {"木": "土", "火": "金", "土": "水", "金": "木", "水": "火"}
        return ke_relation.get(wx1) == wx2 or ke_relation.get(wx2) == wx1

    def _get_tongguan_wuxing(self, wx1: str, wx2: str) -> str:
        """获取通关五行"""
        # 木土对峙，金通关（木生金，金生土）
        # 木金对峙，水通关（金生水，水生木）
        # 水火对峙，木通关（水生木，木生火）
        ...
        key1 = (wx1, wx2)
        key2 = (wx2, wx1)
        return tongguan_map.get(key1) or tongguan_map.get(key2, "")
